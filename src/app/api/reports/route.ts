...
import { getReportInputErrorReason, parseReportInput } from "@/lib/domain/reports";
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ ok: false, reason: getReportInputErrorReason(parsed.error) }, { status: 400 });
  }
