...
export type WorstCarsModuleData = {
  worstCars: DashboardData["worstCars"];
  carExplorer: { options: CarExplorerOption[] };
};
...
export async function getCarSeriesModule(search: DashboardModuleSearch, now = new Date()): Promise<CarSeriesModuleData> {
  ...
  const { data, error } = await supabase.rpc("dashboard_car_series_v2", {
    input_start: window.start.toISOString(),
    input_end: window.end.toISOString(),
    input_lines: selectedLinesArg(search.lines),
    input_car_series: selectedCarSeriesArg(search.carSeries),
  });
...
export async function getWorstCarsModule(search: DashboardModuleSearch, now = new Date()): Promise<WorstCarsModuleData> {
  ...
  if (!supabase) {
    ...
    return { worstCars: fallback.worstCars, carExplorer: { options: fallback.carExplorer.options } };
  }
  const window = getRangeWindow(search.range, now);
  const { data, error } = await supabase.rpc("dashboard_car_summaries_v2", {
    input_start: window.start.toISOString(),
    input_end: window.end.toISOString(),
    input_lines: selectedLinesArg(search.lines),
    input_car_series: selectedCarSeriesArg(search.carSeries),
...
async function getSqlLineSummaries(search: DashboardModuleSearch, now: Date) {
  ...
  const { data, error } = await supabase.rpc("dashboard_line_summaries_v2", {
...
async function getSqlLineCarReports(search: DashboardModuleSearch, now: Date) {
  ...
  if (!supabase) return null;
  const window = getRangeWindow(search.range, now);
  const { data, error } = await supabase.rpc("dashboard_line_car_reports_v2", {
    input_start: window.start.toISOString(),
    input_end: window.end.toISOString(),
    input_lines: selectedLinesArg(search.lines),
    input_car_series: selectedCarSeriesArg(search.carSeries),
...
async function getSqlHeatTrend(search: DashboardModuleSearch, now: Date) {
  ...
  const [trendResult, bucketRows] = await Promise.all([
    supabase.rpc("dashboard_heat_trend_v2", {
...
export async function getCarDetailModule(search: DashboardModuleSearch, car: string, now = new Date()): Promise<CarExplorerSelection | null> {
  ...
  const [summaryResult, historyResult] = await Promise.all([
    supabase.rpc("dashboard_car_summaries_v2", {
      input_start: window.start.toISOString(),
      input_end: window.end.toISOString(),
      input_lines: selectedLinesArg(search.lines),
      input_car_series: selectedCarSeriesArg(search.carSeries),
    ...
    supabase.rpc("dashboard_car_histories_v2", {
      input_start: window.start.toISOString(),
      input_end: window.end.toISOString(),
      input_bucket_seconds: getBucketSeconds(search.range, "chart"),
      input_lines: selectedLinesArg(search.lines),
  ...
  const summaryRow = (summaryResult.data as CarSummaryRow[] | null)?.[0];
  if (!summaryRow) return null;
  const option = toCarExplorerOption(summaryRow);
  const counts = new Map<number, number>();
  for (const row of (historyResult.data ?? []) as AllCarHistoryRow[]) {
    counts.set(new Date(row.bucket_start).getTime(), row.reports);
  }
  return {
    ...option,
    history: buckets.map((bucket) => ({
      label: bucket.label,
      reports: counts.get(bucket.start.getTime()) ?? 0,
    })),
  };
}
...
function selectedCarSeriesArg(series: number[] | undefined) {
