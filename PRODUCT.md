...
- Explorar: browse charts, confidence indicators, car/platform rankings, line detail, and shareable evidence cards.
...
A station interchange therefore represents one separate platform identity per line. For example, Sol on L1 and Sol on L3 are distinct platform-report locations. Direction is not modeled.

Legacy report payloads that omit the location kind are interpreted as `car` reports.

The station catalogue is a closed vocabulary sourced from current CRTM Metro station information. The current product scope is L1-L12; Ramal is intentionally outside this scope.
...
- The car/platform control must expose native radio semantics.
...
- Rate limiting by private abuse key is shared across car and platform reports.
...
- `/explorar`: dashboard with global line, car-series, and time-range filters, charts, confidence indicators, car/platform evidence, line detail, and share cards.
...
Platform reports are included in global report-volume evidence:

- Home 24-hour report count.
- Home recent reports.
- Total reports by day.
- Report volume by line.
- Line evolution by report count.
- Distribution by hour.

Platform reports are excluded from car/fleet-specific calculations:

- Distinct cars reported.
...
When a car-series filter is active, global charts become series-specific and therefore exclude platform reports. Platform-specific charts ignore the car-series filter and continue to honor the selected time range and Metro lines.
...
- Lines with the most platform heat reports, counting `Calor + Infierno` platform reports only.
- Distinct cars reported by line.
- Reports by car series.
- Worst cars.
- Explore car.
- Termo indicator trend over time, based only on car reports.
- Distribution by hour from 5 to 23.
- Fleet coverage, based only on identified car reports.
- Line summary cards whose generic report volume may include platforms while car-fleet score diagnostics remain car-only.
- Line car-detail modal.
...
Local development should include realistic train-car seed data. Platform reporting must also work correctly in memory fallback even if local seed data has no platform reports.
