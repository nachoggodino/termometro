...
-- higher IP-only ceiling for v3 so rotating User-Agent values cannot create infinite quota.
...
-- preserves quota/duplicate continuity with reports created before this migration.
update public.reports
set abuse_key = null
where abuse_key is not null
  and created_at < pg_catalog.now() - interval '30 minutes';

-- Keep the legacy RPC signature callable by old application instances during a
-- DB-first / rolling deployment, but move new legacy writes to the private event
-- store so old and new instances share the same per-origin quota and locks.
...
begin
  ...
  -- Count both new private events and pre-migration report-row hashes so a DB/app
  -- rolling deployment cannot reset a user's current quota.
